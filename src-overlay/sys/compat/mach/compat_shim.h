 ...
 * &machport_filtops).
 *
 * Guarded: built into the kernel (options COMPAT_MACH), <sys/event.h> is
 * the NEXTBSD patched one that already defines EVFILT_MACHPORT (kernel
 * patch 0003), so this would be a redefinition. The #ifndef lets the same
 * shim serve both the out-of-tree kmod build (stock event.h) and the
 * in-tree build (patched event.h).
 */
