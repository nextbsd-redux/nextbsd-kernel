 ...
 *		Nothing locked.
 *
 *	An invalid port is not an error here; it is a no-op. Original Mach put
 *	that check on the CALLER -- every generated server guarded the call as
 *	`if (IP_VALID(p)) ipc_port_release_send(p);` and this routine asserted.
 *	Apple later moved the check into the callee, and modern MIG accordingly
 *	emits the call unguarded. Our servers are generated by that MIG (#125),
 *	so keeping the assert here would panic the moment any of the 14 emitted
 *	call sites in host_priv/task/vm_map saw a null or dead port -- which is
 *	an ordinary thing for a client to send. See #136.
 *
 *	This is behaviour-preserving, not a relaxation: IP_VALID() excludes both
 *	IP_NULL and IP_DEAD, so returning early does exactly what the frozen
 *	servers' caller-side guard did.
 */
